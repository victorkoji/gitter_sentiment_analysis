	...
	def getSearch(self, type_repository, tipo_busca):
		for type, repository in type_repository.items():
			for name_project, url in repository.items():
				...
				if not os.path.exists(f"../ChatRooms/{type}/{name_project}/Github-data"):
					os.makedirs(f"../ChatRooms/{type}/{name_project}/Github-data")

				if data:
					# Escreve os dados para o caminho informado
					Util.write_json(data, f"../ChatRooms/{type}/{name_project}/Github-data/{tipo_busca}.json")
					Util.transformJsonToCsv(f"../ChatRooms/{type}/{name_project}/Github-data/{tipo_busca}.json")
